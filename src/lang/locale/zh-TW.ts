// 繁體中文

export default {
    // UI Language
    "Plugin Language": "插件語言",
    "Interface Language": "界面語言",
    "Choose the language for plugin interface": "選擇插件的界面語言",

    // plugin.ts
    "Open word search panel": "開啟查字面板",
    "Open new word panel": "開啟新字面板",
    "Open statistics": "開啟統計",
    "Open as Reading View": "開啟為閱讀模式",
    "Search word": "查詢單字/片語",
    "Refresh Word Database": "重新整理單字資料庫",
    "Refresh Review Database": "重新整理復習資料庫",

    // DataPanelView.ts
    "Data Panel": "單字列表",

    // SearchPanelView.ts
    "Search Panel": "查字面板",

    // SearchPanel.vue
    "Search": "搜索",

    // ReadingView.ts
    "Finish Reading": "完成閱讀",
    "Return to Markdown": "返回Markdown模式",
    "paragraph": "段落",
    "page": "頁",
    "All": "全部",

    // LearnPanelView.ts
    "Learning New Words": "學習新單字",

    // LearnPanel.vue
    "Expression": "表達",
    "A word or a phrase": "輸入一個單字或片語",
    "Meaning": "含義",
    "A short definition": "給一個簡短的定義",
    "Please input a word/phrase": "輸入一個單字或片語",
    "A short definition is needed": "簡短地描述一下吧",
    "Type": "類別",
    "Word": "單字",
    "Phrase": "片語",
    "Status": "學習狀態",
    "Ignore": "忽略",
    "Learning": "新學",
    "Familiar": "熟悉",
    "Known": "掌握",
    "Learned": "會了",
    "Tags": "標籤",
    "Input or select some tags": "輸入或選擇若干標籤",
    "Notes": "筆記",
    "Create": "新增",
    "Write a new note": "記一則筆記吧",
    "Sentences": "例句",
    "Origin sentence": "源句",
    "Translation (Optional)": "翻譯(可選)",
    "Origin (Optional)": "出處(可選)",
    "Expression is empty!": "單字忘輸入了吧",
    "It looks more like a PHRASE than a WORD": "這看起來更像是個片語而不是單字",
    "Meaning is empty": "含義不應為空",
    "Submit": "提交",

    // Test.vue
    "Day Ignore": "單日-忽略",
    "Day Non-Ignore": "單日-學習",
    // "Single day": "單日",
    "Accumulated": "累計",
    "Statistics": "統計",

    // settings - storage
    "Data Storage": "數據存儲",
    "Storage Type": "存儲方式",
    "JSON file supports Obsidian Sync with high performance": "JSON 文件支持 Obsidian 同步，性能更高",
    "JSON File (Recommended)": "JSON 文件（推薦）",
    "IndexedDB (Local Only)": "IndexedDB（僅本地）",
    "Remote Server": "遠程伺服器",
    "JSON File Path": "JSON 文件路徑",
    "Path relative to vault root, e.g. 'Language Learner/words.json'": "相對於庫根目錄的路徑，如 'Language Learner/words.json'",
    "Sync Debounce": "同步防抖",
    "Delay before syncing to JSON file (ms)": "同步到 JSON 文件的延遲（毫秒）",

    // settings
    "Use Server": "使用伺服器",
    "Use a seperated backend server": "使用單獨的後端伺服器(目前只對作者有用)",
    "Server Port": "伺服器通訊埠",
    'It should be same as "PORT" variable in .env file of server':
        '這應該和伺服器.env文件中的PORT欄位一致',
    "Wrong port format": "埠格式不對",

    "Language": "語言選擇",
    "Native": "母語",
    "Foreign": "外語",
    "Chinese": "中",
    "English": "英",
    "Japanese": "日",
    "Korean": "韓",
    "French": "法",
    "Deutsch": "德",
    "Spanish": "西",

    "Translate": "查字",
    "Word Select": "劃詞翻譯",
    "Press function key and select text to translate": "按下功能鍵同時劃詞或雙擊單字即可開啟翻譯",
    "Disable": "關閉",
    "Dictionary Height": "詞典預設展開高度",
    "Dictionaries": "詞典",
    "Youdao": "有道詞典",
    "Cambridge": "劍橋詞典",
    "Hujiang": "滬江小D",
    "Iciba": "金山詞霸",

    "Word Database Path": "單字資料庫路徑",
    "Choose a md file as word database for auto-completion": "選擇一個md文件作為單字資料庫以自動補全",
    "Review Database Path": "復習資料庫路徑",
    "Choose a md file as review database for spaced-repetition": "選擇一個md文件作為復習資料庫以間隔復習",
    "Last review sync": "上次更新時間",
    "Last time the review database was updated": "上次更新復習資料庫的時間",
    "Destroy Database": "銷燬資料庫",
    "Destroy all stuff and start over": "銷燬一切從頭開始",
    "Are you sure you want to destroy your database?": "刪了庫，可就回不了頭啦",
    "Destroy": "銷燬",
    "Yes": "確定",
    "No!!!": "不要!!!",
    "Reset": "重置",
    "Are you sure you want to reset last sync time?": "確定要重置重新整理時間嗎?",

    "Text Database": "文字資料庫",
    "Auto refresh": "自動重新整理資料庫",
    "Auto refresh database when submitting": "當提交單字時自動重新整理文字資料庫",


    "IndexDB Database": "IndexDB 資料庫",
    "Database Name": "資料庫名稱",
    "Reopen": "重啟",
    // "Change": "Change",
    "Reopen DB after changing database name": "切換資料庫後點擊按鈕重啟資料庫",
    "Import & Export": "匯入導出數據",
    "Warning: Import will override current database": "警告: 匯入會覆蓋掉目前的資料庫",
    "Import": "匯入",
    "Export": "導出",
    "Get all ignores": "獲取所有無視單字",
    "Get all non-ignores": "獲取所有非無視單字",
    "Copied to the clipboard": "已複製到剪貼簿",
    "Export Word": "導出單字",
    "Export Word and Phrase": "導出單字和片語",

    "Reading Mode": "閱讀模式",
    "Default Paragraphs": "每頁預設段數",
    "Use Machine Translation": "使用機器翻譯",
    "Auto translate sentences": "用機器翻譯自動翻譯例句",
    "Open count bar": "開啟單字計數條(較影響開啟文章時的效能)",
    "Count the word number of different type of article": "計算文章中各種詞的數目和佔比",
    "Font Size": "字型大小",
    "Like 15px or 1.5em": "比如 15px 或 1.5em",
    "Font Family": "字型",
    "Line Height": "行高",

    "Auto Completion": "自動補全",
    "Column delimiter": "列分隔符",
    "Comma": "英文逗號 ,",
    "Tab": "製表符 tab",
    "Pipe": "豎線 |",
    "Review": "復習",
    "Accent": "口音",
    "Choose your preferred accent": "選擇你更喜歡的口音",
    "American": "美式",
    "British": "英式",

    "As Server": "成為伺服器",
    "Self as Server": "開啟伺服器功能",
    "Make plugin a server and interact with chrome extension": "外掛開啟一個伺服器提供API，由此可以與Google瀏覽器外掛互動",
    "when changing port, you should restart the server": "如果改變埠，請重啟伺服器",

    // Tags
    "Filter by tags": "按標籤篩選",
    "Select multiple to filter": "多選進行篩選",
    "Create new tag": "創建新標籤",
    "Tag name": "標籤名稱",
    "Add": "新增",
    "selected": "已選",

    // AnkiConnect
    "AnkiConnect": "AnkiConnect",
    "Enable AnkiConnect": "啟用 AnkiConnect",
    "Enable integration": "啟用 Anki 整合",
    "Anki Host": "Anki 主機",
    "AnkiConnect server host": "AnkiConnect 伺服器位址",
    "Anki Port": "Anki 連接埠",
    "AnkiConnect server port (default 8765)": "AnkiConnect 伺服器連接埠（預設 8765）",
    "API Key": "API 密鑰",
    "AnkiConnect API key (optional)": "AnkiConnect API 密鑰（可選）",
    "Default Deck": "預設牌組",
    "Default Anki deck for new cards": "新卡片的預設 Anki 牌組",
    "Note Model": "筆記模板",
    "Anki note model (e.g., Basic, Cloze)": "Anki 筆記模板（如 Basic、Cloze）",
    "Test Connection": "測試連線",
    "Test": "測試",
    "AnkiConnect connected successfully": "AnkiConnect 連線成功",
    "AnkiConnect connection failed": "AnkiConnect 連線失敗",
    "AnkiConnect error": "AnkiConnect 錯誤",
    "AnkiConnect is not enabled": "AnkiConnect 未啟用",
    "Enable AnkiConnect in settings first": "請先在設定中啟用 AnkiConnect",
    "Connected to Anki": "已連線到 Anki",
    "Not connected": "未連線",
    "Retry": "重試",
    "Export to Anki": "匯出到 Anki",
    "Refresh": "重新整理",
    "Deck": "牌組",
    "Model": "模板",
    "Filter": "篩選",
    "words": "個單字",
    "Select All": "全選",
    "Previous": "上一頁",
    "Next": "下一頁",
    "Exporting...": "匯出中...",
    "Successfully exported": "成功匯出",
    "Exported": "已匯出",
    "Failed": "失敗",
    "Export failed": "匯出失敗",
    "Anki Export": "Anki 匯出",
    "Open Anki Export": "開啟 Anki 匯出",

    // Smart Review System
    "Smart Review": "智能複習",
    "Open Smart Review": "打開智能複習",
    "New": "新詞",
    "Remaining": "剩餘",
    "Click to show answer": "點擊查看答案",
    "All done!": "全部完成！",
    "No cards due for review today.": "今天沒有需要複習的卡片。",
    "Import from word database": "從單詞庫導入",
    "Skip": "跳過",
    "Stats": "統計",
    "Again": "忘記",
    "Hard": "困難",
    "Good": "良好",
    "Easy": "簡單",
    "Again - < 1m": "忘記 - < 1分鐘",
    "Sentence": "句子",
    "Settings saved": "設置已保存",
    "Imported ${imported} cards from database": "從數據庫導入了 ${imported} 張卡片",
    "No new cards to import": "沒有新卡片可導入",
    "Review system not initialized": "複習系統未初始化",
    "Scheduler not initialized": "調度器未初始化",
    "No words found in database": "數據庫中沒有單詞",
    "No learnable words found (status must be Learning or higher)": "沒有可學習的單詞（狀態必須是學習中或以上）",
    "All learnable words already imported": "所有可學習的單詞已導入",
    "Review Statistics": "複習統計",
    "Total Cards": "總卡片數",
    "Mastered": "已掌握",
    "Due Today": "今日到期",
    "Today's Progress": "今日進度",
    "Data Management": "數據管理",
    "Clear All Cards": "清除所有卡片",
    "This will delete all review cards and allow re-import": "這將刪除所有複習卡片，允許重新導入",
    "Are you sure you want to delete all review cards?": "確定要刪除所有複習卡片嗎？",
    "All cards cleared. You can now re-import.": "所有卡片已清除。現在可以重新導入。",
    "Failed to clear cards": "清除卡片失敗",
    "Import failed: ": "導入失敗：",
    "Review Settings": "複習設置",
    "Default Mode": "默認模式",
    "Word Only": "僅單詞",
    "Sentence Only": "僅句子",
    "Mixed": "混合",
    "Daily New Cards": "每日新卡片",
    "Daily Review Cards": "每日複習卡片",
    "Word Card Template": "單詞卡片模板",
    "Front Fields": "正面字段",
    "Back Fields": "反面字段",
    "Pronunciation": "音標",
    "Example Sentence": "例句",
    "Example": "例句",
    "Sentence Card Template": "句子卡片模板",
    "Highlight Style": "高亮樣式",
    "None": "無",
    "Bold": "加粗",
    "Underline": "下劃線",
    "Color": "顏色",
    "Highlight Color": "高亮顏色",
    "Interface Settings": "界面設置",
    "Show Answer Button": "顯示答案按鈕",
    "Auto Play Audio": "自動播放音頻",
    "Algorithm Parameters": "算法參數",
    "Easy Bonus": "簡單獎勵係數",
    "Hard Interval": "困難間隔係數",

    // Heatmap Panel
    "Learning Heatmap": "學習熱力圖",
    "Total Words": "總單詞數",
    "Current Streak": "當前連續",
    "Longest Streak": "最長連續",
    "Less": "少",
    "More": "多",
    "No words recorded on this date": "該日期沒有記錄單詞",
    "Created": "創建",
    "Modified": "修改",

    // Heatmap Panel - View Modes
    "Year View": "年視圖",
    "Month Grid": "月份網格",
    "Multi Year": "多年對比",
    "Show years": "顯示年份",
    "Last 3 years": "最近3年",
    "Last 5 years": "最近5年",
    "All years": "全部年份",
    "days": "天",
    "No data available": "暫無數據",
    "Failed to load data": "加載數據失敗",

    // Month abbreviations
    "Jan": "1月",
    "Feb": "2月",
    "Mar": "3月",
    "Apr": "4月",
    "May": "5月",
    "Jun": "6月",
    "Jul": "7月",
    "Aug": "8月",
    "Sep": "9月",
    "Oct": "10月",
    "Nov": "11月",
    "Dec": "12月",

    // PDF Export
    "Export to PDF": "匯出為 PDF",
    "Export PDF": "匯出 PDF",
    "Title": "標題",
    "Subtitle": "副標題",
    "Date": "日期",
    "Article title": "文章標題",
    "Optional": "可選",
    "Show phonetic": "顯示音標",
    "Words to include": "包含的單字",
    "Cancel": "取消",
    "Please open an article in reading mode first": "請先開啟一篇文章並切換到閱讀模式",
    "PDF export failed": "PDF 匯出失敗",
    "Please select at least one word": "請至少選擇一個單字",
    "Preview": "預覽",
};
