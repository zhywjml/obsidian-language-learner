...
**在线词典排序优化** 📚
- 将设置中的"词典"更名为"在线词典"，与"离线词典"区分更清晰
- 新增在线词典排序功能
  - 支持上移/下移按钮调整词典顺序
  - 显示序号（1, 2, 3...）直观展示当前优先级
  - 添加"重置排序"按钮恢复默认顺序
- 词典顺序调整后，查词面板即时刷新
...
**深色模式修复** 🌙
- 修复插件样式污染 Obsidian 全局深色模式的问题
  - 修复热力图组件 (`MonthGrid.vue`, `HeatmapPanel.vue`, `SimplifiedYearHeatmap.vue`) 的全局 `.theme-dark` 样式污染
  - 修复阅读模式在深色模式下文字显示异常的问题
  - 修复 Obsidian 设置面板左侧栏在深色模式下变黑的问题
- 新增离线词典深色模式适配
  - 支持覆盖词典 HTML 中的硬编码黑色/深灰色文字
  - 支持覆盖词典 HTML 中的白色背景
  - 图片自动调暗，避免刺眼
  - 表格边框和表头背景适配深色主题
...
- 统一所有悬浮下拉菜单的交互体验
  ...
  - 学习新单词面板 - 标签选项框
  - 离线词典 - 词典选择选项框
- 采用父容器包裹 + 200ms 延迟关闭机制，解决鼠标移动时菜单闪烁问题
- 优化菜单与触发按钮之间的间隙处理
...
**智能复习系统（测试版）** 🧠
- 新增基于 SM-2 算法的智能复习系统，类似 Anki 的闪卡功能
- 支持从单词库导入单词进行间隔重复复习
- 卡片正面显示单词，背面显示含义和例句
- 支持四种难度评分：忘记/困难/良好/简单
- 自动计算下次复习时间
- 可设置每日新卡片和复习卡片上限

> ⚠️ **注意**：此功能仍在测试阶段，可能存在不稳定情况。欢迎反馈问题。

---

**性能优化**
- 新增 AC 自动机缓存，避免重复构建自动机，提升短语查询性能
- 新增 Web Worker 文本解析支持，长文章解析移至后台线程避免阻塞 UI
- 新增样式滑动条防抖（100ms），提升交互流畅度
- 优化 processContent Markdown 渲染逻辑，减少正则替换次数
- 使用 requestIdleCallback 延迟非关键统计任务

**构建优化**
- Web Worker 代码内联到主包，简化部署流程
